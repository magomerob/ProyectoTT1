 ...
 * @brief Implementación de DEInteg.
 * @version 0.1
 * @date 2025-05-17
 * 
...
{
    double eps = pow(2,-52);// C++ equivalent of Matlab's eps
    int i, ip1;
    double epsilon, told, del, absdel, tend, nostep, kle4, releps, abseps, delsgn, x, h, hi, ki, kold, temp1, term, psijm1, gamma, eta, p5eps, ifail, round, sum, absh, kp1, kp2, km1, km2, ns, nsp1, hold, k, hnew, realns, im1, temp2, reali, temp4, nsm2, limit1, temp5, temp3, nsp2, limit2, temp6, tau, xold,erkm2,erkm1,erk, err, knew, erkp1,r, rho2;
    bool stiff, OldPermit, start, crash, phase1, nornd, success;
    // maxnum = 500;
    double twou  = 2*eps;
    double fouru = 4*eps;
    ...
    // return y, if output time equals input time
    ...
    if (abs(x-t) >= absdel){
        ...
        yout = y + yout*hi;
        y    = yout;
        State_    = DE_STATE::DE_DONE; // Set return y code
    ...
    if (!success){
        ...
        // tolerance and return y if estimated step size is too small
        ...
        //
        // End block 3, return y to start of block 1
        //
